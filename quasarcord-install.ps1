# ==============================================================================
#  QuasarCord — Installeur utilisateur (PowerShell autonome)
#  
#  Ce script fait TOUT automatiquement :
#  1. Télécharge EquilotlCli.exe (outil d'injection graphique)
#  2. Télécharge les fichiers QuasarCord compilés depuis GitHub
#  3. Lance l'interface graphique pour choisir votre Discord cible
#  4. Injecte QuasarCord dans Discord
#
...
function Write-Banner {
    ...
    Write-Host "  ║          QUASARCORD  INSTALLER            ║" -ForegroundColor Cyan
...
if (Test-Path $EquilotlExe) {
    ...
    try {
        $head = Invoke-WebRequest -Uri $EquilotlUrl -Method Head -UseBasicParsing `
            -Headers @{ "User-Agent" = "QuasarCord-Installer/2.0" }
        $cl = $head.Headers["Content-Length"]; $remoteSize = if ($cl) { [long]$cl } else { 0 }
...
# ── [2/3] Télécharger les fichiers QuasarCord ──────────────────────────────────
Write-Step 2 3 "Téléchargement des fichiers QuasarCord depuis GitHub..."

try {
    $apiUrl   = "https://api.github.com/repos/$QuasarCordRepo/releases/latest"
    $release  = Invoke-RestMethod -Uri $apiUrl -UseBasicParsing `
        -Headers @{ "User-Agent" = "QuasarCord-Installer/2.0"; "Accept" = "application/vnd.github.v3+json" }

    $version  = $release.tag_name
    $distAsset = $release.assets | Where-Object { $_.name -eq "quasarcord-dist.zip" } | Select-Object -First 1

    if (-not $distAsset) {
        Write-Fail "Fichier 'quasarcord-dist.zip' introuvable dans la release $version.`n           Contactez le support QuasarCord."
    }

    Write-Host "          Version : $version" -ForegroundColor DarkGray
    Write-Host "          Téléchargement en cours..." -ForegroundColor DarkGray

    $zipPath = Join-Path $InstallDir "quasarcord-dist.zip"
    ...
    Write-OK "QuasarCord $version prêt à être injecté !"
} catch {
    Write-Fail "Échec du téléchargement QuasarCord.`n           Détail : $_"
}
...
Write-Host "          │  Sélectionnez le Discord où injecter QuasarCord. │" -ForegroundColor DarkCyan
Write-Host "          └─────────────────────────────────────────────────┘" -ForegroundColor DarkCyan
Write-Host ""
...
Write-Host "  ║  QuasarCord installé avec succès !                    ║" -ForegroundColor Green
Write-Host "  ║                                                      ║" -ForegroundColor Green
Write-Host "  ║  → Redémarrez Discord pour appliquer QuasarCord.      ║" -ForegroundColor Green
Write-Host "  ╚══════════════════════════════════════════════════════╝" -ForegroundColor Green
Write-Host ""
Write-Host "  Pour désinstaller : exécutez quasarcord-uninstall.bat" -ForegroundColor DarkGray
Write-Host ""
