# build-installer.ps1 - Compile QuasarCord-Installer.exe
...
$OutExe = Join-Path $OutDir "QuasarCord-Installer.exe"

Write-Host ""
Write-Host "  [QuasarCord] Compiling installer..." -ForegroundColor Cyan
...
if ($hasDotnet) {
    ...
    & dotnet publish "$SrcDir\QuasarCordInstaller.csproj" `
        -c Release `
        -o $OutDir `
        --nologo `
        -v quiet `
    ...
    $built = Get-ChildItem $OutDir -Recurse -Filter "QuasarCord-Installer.exe" -ErrorAction SilentlyContinue | Select-Object -First 1
...
# Method 2: csc.exe (.NET Framework - always present on Windows)
    ...
    $ico    = Join-Path $Root "quasarcord.ico"
...
if (Test-Path $OutExe) {
    ...
    Write-Host ""
    Write-Host "  OK  QuasarCord-Installer.exe compiled ($size KB)" -ForegroundColor Green
    Write-Host "    -> $OutExe" -ForegroundColor DarkGray
    Write-Host ""
} else {
    Write-Host "  [ERROR] QuasarCord-Installer.exe not found after compilation." -ForegroundColor Red
    exit 1
}
