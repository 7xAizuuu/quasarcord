...
namespace QuasarCordInstaller
{
    ...
    public class LauncherForm : Form
    {
        ...
        private QuasarCordBackend _backend;

        public LauncherForm()
        {
            this.Text = "QuasarCord Installer";
            ...
            var iconStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("QuasarCordInstaller.icon.ico");
        ...
        private async void InitializeWebView()
        {
            ...
            _backend = new QuasarCordBackend(this, _webView);
            ...
            using (var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("QuasarCordInstaller.index.html"))
            using (var reader = new StreamReader(stream))
            {
                ...
                try {
                    using (var iconStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("QuasarCordInstaller.icon.ico"))
                    {
    ...
    [ComVisible(true)]
    public class QuasarCordBackend
    {
        ...
        const string GITHUB_REPO = "7xAizuuu/quasarcord";
        const string DIST_ZIP = "quasarcord-dist.zip";

        public QuasarCordBackend(LauncherForm form, WebView2 webView)
        {
            _form = form;
            _webView = webView;
            ...
            _distDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "QuasarCord", "dist");
        }
        ...
        public async Task<bool> IsInjected(string path)
        {
            return await Task.Run(() => {
                ...
                return Directory.Exists(appDir) && File.Exists(pkgPath) && File.ReadAllText(pkgPath).Contains("\"quasarcord\"");
            });
        }
        ...
        private async Task EnsureDistAsync()
        {
            ...
            _http.DefaultRequestHeaders.Add("User-Agent", "QuasarCord-Installer/2.0");
            ...
            var tmpZip = Path.Combine(Path.GetTempPath(), "quasarcord-dist.zip");
                ...
                using (var fs = new FileStream(tmpZip, FileMode.Create, FileAccess.Write, FileShare.None, 81920, true))
                {
                    ...
                    while ((read = await contentStream.ReadAsync(buffer, 0, buffer.Length)) > 0)
                    {
                        ...
                        if (percent - lastReportedPercent >= 0.5 || percent >= 100.0)
                        {
                            ...
                            SetProgress(overallPercent, "Downloading QuasarCord...", readMB, totalMB);
                        }
                    }
                }
            }
        ...
        private void DoInject(string resPath)
        {
            ...
            if (Directory.Exists(appDir))
            {
                ...
                if (File.Exists(pkg) && File.ReadAllText(pkg).Contains("\"quasarcord\"")) {
                    WriteLoader(appDir);
        ...
        private void DoUninject(string resPath)
        {
            ...
            // 3. Clean up QuasarCord-specific assets (folders we added)
        ...
        private void WriteLoader(string appDir)
        {
            ...
            // _distDir = %LOCALAPPDATA%\QuasarCord\dist\
            ...
            File.WriteAllText(Path.Combine(appDir, "package.json"), "{\"name\":\"quasarcord\",\"main\":\"index.js\"}");
            File.WriteAllText(Path.Combine(appDir, "index.js"),
                ...
                $"if (!fs.existsSync(patcherPath)) throw new Error('[QuasarCord] patcher.js not found. Expected at: ' + primary);\n" +
                $"require(patcherPath);\n"
            );
        }
