...
import { t } from "../autoTranslateQuasarCord";
import "./styles.css";
...
const MI_TOKEN_CACHE_KEY = "quasarcord-mi-token-cache";
...
function ContextMenuPortal(props: CtxMenuProps) {
    ...
    const [container] = React.useState(() => {
        const el = document.getElementById("quasarcord-mi-ctx-root") ?? document.createElement("div");
        el.id = "quasarcord-mi-ctx-root";
...
export default definePlugin({
    ...
    async start() {
        ...
        captureCurrentToken();
        addHeaderBarButton("quasarcord-multi-instance", () => <MultiInstanceButton />, 9);
    },

    stop() {
        removeHeaderBarButton("quasarcord-multi-instance");
        if (this._fluxUnsub) { this._fluxUnsub(); this._fluxUnsub = null; }
        const root = document.getElementById("quasarcord-mi-ctx-root");
        if (root) root.remove();
    },
});
