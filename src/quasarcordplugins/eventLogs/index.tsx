...
import { t, useTranslation } from "../autoTranslateQuasarCord";
import "./styles.css";
...
function loadPersistLogs() {
    try {
        const s = localStorage.getItem("quasarcord_logs");
        if (s) {
...
function savePersistLogs() {
    try {
        ...
        localStorage.setItem("quasarcord_logs", JSON.stringify(toSave));
    } catch { }
}
...
function LogRow({ e }: { e: LogEntry; }) {
    ...
    const onDoubleClick = () => {
        ...
        try {
            ...
            console.error("QuasarCord Navigation Error:", err);
...
function LogsModal({ rootProps }: { rootProps: any; }) {
    ...
    const saveAsTxt = useCallback(() => {
        try {
            ...
            a.href = url;
            a.download = `quasarcord_logs_${new Date().toISOString().slice(0, 10)}.txt`;
            a.click();
...
export default definePlugin({
    ...
    start() {
        ...
        loadPersistLogs();
        addHeaderBarButton("quasarcord-event-logs", () => <LogsButton />, 7);
        subscribeToEvents();
    },
    stop() {
        removeHeaderBarButton("quasarcord-event-logs");
