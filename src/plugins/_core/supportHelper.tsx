...
import { platformName } from "@quasarcordplugins/equicordHelper/utils";
...
export default definePlugin({
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        // Groupe 2 : Message "QuasarCord developers"
        const quasarcordGroupIds = [
        ...
        const isQuasarCordGroup = quasarcordGroupIds.includes(userId);

        if (!isOriginalDev && !isQuasarCordGroup) return null;
        if (RelationshipStore.isFriend(userId) || isAnyPluginDev(UserStore.getCurrentUser()?.id)) return null;

        const developerText = isOriginalDev ? "Equicord & Vencord plugin developers" : "QuasarCord developers";
