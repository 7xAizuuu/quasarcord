...
// Note: quasarcordTray removed — QuasarCord injects silently into Discord,
...
console.log("[QuasarCord] Starting up...");
...
if (!IS_VANILLA) {
            ...
            if (options?.webPreferences?.preload && options.title) {
                ...
                if (process.platform === "win32" && winMaterial && winMaterial !== "none") {
                    try {
                        ...
                        if (!applied) {
                            console.warn("[QuasarCord] No background material API available on this system");
                        }
                    } catch (e) {
                        console.error("[QuasarCord] setBackgroundMaterial failed:", e);
                    }
                }
    ...
    process.env.DATA_DIR = join(app.getPath("userData"), "..", "QuasarCord");
    ...
    console.log("[QuasarCord] Running in vanilla mode. Not loading QuasarCord");
}

console.log("[QuasarCord] Loading original Discord app.asar");
