...
function createQuasarCordMenuItems(): MenuItemConstructorOptions[] {
    return [
        {
            label: "QuasarCord",
            submenu: [
                {
                    label: "About QuasarCord",
                    click: () => openAboutWindow()
                },
                {
                    label: cachedUpdateAvailable ? "Update QuasarCord" : "Check for Updates",
...
export function patchTrayMenu(): void {
    // On ne patch plus le menu tray — QuasarCord gère son propre tray dans patcher.ts
}
