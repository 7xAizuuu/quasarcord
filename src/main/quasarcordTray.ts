...
/**
 * Initializes the QuasarCord system tray icon.
 ...
 */
export function initQuasarCordTray(injectorPath: string) {
    ...
    app.whenReady().then(() => {
        try {
            ...
            tray.setToolTip("QuasarCord");

            const trayMenu = Menu.buildFromTemplate([
                {
                    label: "Open QuasarCord",
                    click() {
                ...
                { type: "separator" },
                {
                    label: "Uninstall QuasarCord",
                    click() {
                        const uninstallerCandidates = [
                            join(process.env.LOCALAPPDATA || "", "Programs", "quasarcord", "Uninstall QuasarCord.exe"),
                            join(process.env.LOCALAPPDATA || "", "Programs", "QuasarCord", "Uninstall QuasarCord.exe"),
                            join(process.env.PROGRAMFILES || "", "QuasarCord", "Uninstall QuasarCord.exe"),
                        ];
            ...
            console.log("[QuasarCord] Tray created ✓", iconPath ?? "(icon from exe)");
        } catch (e) {
            console.error("[QuasarCord] Failed to create tray:", e);
        }
    });
}
