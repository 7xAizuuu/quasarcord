...
const RELEASES_REPO = "7xAizuuu/quasarcord";
...
const ZIP_FILE = "quasarcord-dist.zip";
...
async function applyUpdates(): Promise<boolean> {
    ...
    try {
        ...
        // Save to temp
        const zipPath = join(app.getPath("temp"), `quasarcord-update-${Date.now()}.zip`);
...
app.on("before-quit", (event) => {
    ...
    console.log("[QuasarCord] Applying pending update before quit...");

    const safetyTimeout = setTimeout(() => {
        console.error("[QuasarCord] Update on quit timed out — forcing exit.");
        app.exit(0);
    }, 45_000);

    applyUpdates()
        .then(ok => {
            if (ok) console.log("[QuasarCord] Update applied successfully on quit.");
            else    console.warn("[QuasarCord] Update on quit returned false.");
        })
        .catch(err => {
            console.error("[QuasarCord] Update on quit failed:", err);
        })
        .finally(() => {
