...
export const DATA_DIR = process.env.QUASARCORD_USER_DATA_DIR ?? (
    process.env.DISCORD_USER_DATA_DIR
        ? join(process.env.DISCORD_USER_DATA_DIR, "..", "QuasarCordData", suffix)
        : join(app.getPath("userData"), "..", "QuasarCord", suffix)
);
    ...
    if (!migrated) {
        setTimeout(() => {
            try {
                ...
                console.error("[QuasarCord] Failed to copy prod data:", err);
            }
        }, 5000);
    }
}
