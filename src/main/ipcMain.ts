...
// ── Early start of ghost-server on QuasarCord launch ──────────────
// The server runs in the background from startup to avoid delay on first click
import * as ghostNative from "../quasarcordplugins/ghostClient/native";
(async () => {
    try {
        await (ghostNative as any).init(null);
    } catch (e) {
        console.warn("[QuasarCord] Ghost-server pre-start failed:", e);
    }
})();
...
ipcMain.handle(IpcEvents.OPEN_MONACO_EDITOR, async () => {
    ...
    monacoWin = new BrowserWindow({
        title: "QuasarCord QuickCSS Editor",
...
// ── QuasarCord Relaunch — redémarre l'app Electron complètement ──
// VencordNative.quasarcord.relaunch() envoie RELAUNCH_APP = "QuasarCordRelaunchApp"
// VesktopNative.app.relaunch() envoie aussi RELAUNCH_APP
// Les boutons "Restart" dans les plugins settings (index.tsx) passent par VencordNative.quasarcord.relaunch()
ipcMain.handle(IpcEvents.RELAUNCH_APP, async () => {
    // Dans l'architecture QuasarCord, app.asar est un dossier (pas un vrai .asar),
...
// ── QuasarCord Updater — downloads a Setup.exe from a URL and runs it ──
ipcMain.handle(IpcEvents.QUASARCORD_DOWNLOAD_AND_RUN, async (_, url: string) => {
    ...
    const tmpPath = path.join(os.tmpdir(), "QuasarCordUpdate-Setup.exe");
...
ipcMain.handle(IpcEvents.INSTALL_VB_CABLE, async () => {
    ...
    const tmpDir = path.join(os.tmpdir(), "QuasarCord-VBCable");
    ...
    return new Promise((resolve) => {
        // Commande PowerShell : Téléchargement -> Extraction -> Execution de l'installeur (x64 car QuasarCord est x64)
        const psCmd = `
        ...
        exec(`powershell -Command "${psCmd}"`, (err: any) => {
            if (err) {
                console.error("[QuasarCord] VBCable install failed:", err);
