...
try { _stealthActive = localStorage.getItem("QuasarCord_stealthMode") === "1"; } catch { }
...
function persistStealth(v: boolean) {
    try { v ? localStorage.setItem("QuasarCord_stealthMode", "1") : localStorage.removeItem("QuasarCord_stealthMode"); } catch { }
}

// Éléments NON-React uniquement (titlebar, QuasarCordAI nav)
// NE PAS cacher les entrées settings sidebar — l'utilisateur doit pouvoir accéder aux paramètres
const NON_REACT_SELECTORS = [
    "#quasarcord-titlebar-btn",
    "#quasarcord-titlebar-link-style",
    ".nai-nav-item",
];
...
export function syncStealthBodyClass() {
    try { if (_stealthActive) document.body?.classList.add("quasarcord-stealth"); else document.body?.classList.remove("quasarcord-stealth"); } catch { }
...
if (_stealthActive) {
    ...
    try { document.body?.classList.add("quasarcord-stealth"); } catch { }
}
...
try {
    ...
    window.addEventListener("quasarcord-stealth-change", () => {
...
export function _notifyStealthChange() {
    ...
    window.dispatchEvent(new Event("quasarcord-stealth-change"));
}
...
function HeaderBarButtons() {
    ...
    useEffect(() => {
        ...
        window.addEventListener("quasarcord-stealth-change", listener);
        return () => {
            headerBarListeners.delete(listener);
            stealthListeners.delete(listener);
            window.removeEventListener("quasarcord-stealth-change", listener);
        };
    }, []);
