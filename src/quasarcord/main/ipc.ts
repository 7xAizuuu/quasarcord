...
handleSync(IpcEvents.GET_GIT_HASH, () => QuasarCord_GIT_HASH);
...
// Handler pour VencordNative.quasarcord.relaunch() — utilisé par les boutons Restart dans les settings plugins
...
handleSync(IpcEvents.IS_USING_CUSTOM_VENCORD_DIR, () => !!State.store.QuasarCordDir);
handle(IpcEvents.SHOW_CUSTOM_VENCORD_DIR, async () => {
    const { QuasarCordDir } = State.store;
    if (!QuasarCordDir) return;

    const stats = await stat(QuasarCordDir);
    if (!stats.isDirectory()) return;

    shell.openPath(QuasarCordDir);
});
...
handle(IpcEvents.SELECT_VENCORD_DIR, async (_e, value?: null) => {
    if (value === null) {
        delete State.store.QuasarCordDir;
        return "ok";
    }
