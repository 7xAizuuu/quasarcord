...
const QUASARCORD_RELEASES_REPO = "7xAizuuu/quasarcord";
const QUASARCORD_REPO_URL = `https://github.com/${QUASARCORD_RELEASES_REPO}`;
...
export { QUASARCORD_REPO_URL };

export async function getCommitsSinceLastSeen(
    repoUrl: string,
): Promise<ChangelogEntry[]> {
    // Toujours utiliser le repo QuasarCord, ignorer le repoUrl d'Equicord
    return fetchCommitsBetween(QUASARCORD_RELEASES_REPO, "HEAD~10", "HEAD").catch(() => []);
}
