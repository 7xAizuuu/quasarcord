...
export const ExcludedReasons: Record<"web" | "discordDesktop" | "vesktop" | "equibop" | "desktop" | "dev", string> = {
    ...
    dev: "Developer version of QuasarCord"
};
...
export default function PluginSettings({ premiumOnly = false }: PluginSettingsProps) {
    ...
    const [searchValue, setSearchValue] = useState({ value: "", status: SearchStatus.QUASARCORD });
    ...
    React.useEffect(() => {
        ...
        window.addEventListener("quasarcord-tutorial-detected", handler);
        return () => window.removeEventListener("quasarcord-tutorial-detected", handler);
    }, []);
    ...
    const pluginFilter = useCallback((plugin: typeof Plugins[keyof typeof Plugins], newPluginsSet: Set<string> | null) => {
        ...
        switch (status) {
            ...
            case SearchStatus.ENABLED:
                if (!enabled) return false;
                break;
            case SearchStatus.QUASARCORD:
                if (!PluginMeta[plugin.name].folderName.startsWith("src/quasarcordplugins/")) return false;
                break;
            case SearchStatus.OTHERS:
                if (PluginMeta[plugin.name].folderName.startsWith("src/quasarcordplugins/") || PluginMeta[plugin.name].folderName.startsWith("src/plugins/_")) return false;
    ...
    const { quasarcordPlugins, othersPlugins, requiredPlugins } = useMemo(() => {
        const quasarcordPlugins = [] as JSX.Element[];
        ...
        for (const p of sortedPlugins) {
            ...
            if (isRequired) {
                const tooltipText = p.required || !depMap[p.name]
                    ? "This plugin is required for QuasarCord to function."
                ...
                const isQuasarCord = folderName.startsWith("src/quasarcordplugins/");
                const card = (
                    <PluginCard
                        onRestartNeeded={handleRestartNeeded}
                        disabled={false}
                ...
                if (isQuasarCord) {
                    quasarcordPlugins.push(card);
                } else {
                    othersPlugins.push(card);
                }
            }
        }
        return { quasarcordPlugins, othersPlugins, requiredPlugins };
    ...
    const allPlugins = [...quasarcordPlugins, ...othersPlugins];
    ...
    const quasarcordVisible = quasarcordPlugins.slice(0, Math.min(visibleCount, quasarcordPlugins.length));
    const othersVisible = othersPlugins.slice(0, Math.max(0, visibleCount - quasarcordPlugins.length));
        ...
        <SettingsTab>
                    ...
                    <ErrorBoundary noop>
                        <Select
                            options={[
                                ...
                                { label: "Show QuasarCord Plugins", value: SearchStatus.QUASARCORD },
            ...
            {premiumOnly ? (
                <>
                    ...
                    {quasarcordPlugins.length || othersPlugins.length
                        ? (
                            <div className={cl("grid")}>
                                {[...quasarcordVisible, ...othersVisible].length
                                    ? [...quasarcordVisible, ...othersVisible]
                ...
                <>
                    {quasarcordPlugins.length > 0 && (
                        <>
                            <HeadingTertiary className={Margins.top20}>QuasarCord Plugins</HeadingTertiary>
                            <div className={cl("grid")}>
                                {quasarcordVisible}
                            </div>
                        </>
                    ...
                    {quasarcordPlugins.length === 0 && othersPlugins.length === 0 && (
                        <ExcludedPluginsList search={search} />
                    )}
