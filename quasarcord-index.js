...
// ── CRITIQUE : userData = dossier QuasarCord pour les settings/plugins
...
const quasarcordData = path.join(app.getPath("appData"), "QuasarCord");
app.setPath("userData", quasarcordData);
...
// pour le token Discord stable. Quand QuasarCord charge ce cache, il lit des permissions
...
// AppUserModelId unique — Windows reconnaît QuasarCord comme app séparée de Discord
...
app.once("ready", () => {
    try {
        ...
        //     Seuls les modules vraiment inutiles pour QuasarCord sont bloqués.
        ...
        app.once("browser-window-created", () => {
            try {
                ...
                ses.webRequest.onBeforeRequest(
                    ...
                    (details, callback) => {
                        ...
                        if (isBlocked) {
                            ...
                            console.log("[QuasarCord] Module bloqué (inutile pour QuasarCord):", url.split("/").slice(-2).join("/"));
                ...
                console.log("[QuasarCord] Filtre modules 403 activé ✓");
            } catch (e) {
                console.warn("[QuasarCord] Impossible d'activer le filtre modules:", e.message);
            }
        });
    } catch (e) {
        console.warn("[QuasarCord] FIX modules 403 failed:", e.message);
    }
});
...
try {
    const lsPath = path.join(quasarcordData, "Local Storage", "leveldb");
    if (fs.existsSync(lsPath)) {
        ...
        if (corrupted) {
            console.warn("[QuasarCord] LevelDB localStorage corrompu détecté — réparation...");
            try { fs.rmSync(lsPath, { recursive: true, force: true }); } catch { }
            console.warn("[QuasarCord] LevelDB supprimé — les données localStorage seront récréées");
        }
    }
} catch (e) { console.warn("[QuasarCord] LevelDB check failed:", e.message); }

// Modules bundlés dans quasarcord-dist/modules/
...
try {
    ...
    if (entries.length > 0) {
        ...
        console.log("[QuasarCord] Modules natifs Discord détectés:", discordNativeModulesPath);
    }
} catch (e) {
    console.warn("[QuasarCord] Impossible de détecter les modules natifs Discord:", e.message);
}
...
// Priorité aux modules bundlés (portables, dans quasarcord-dist/modules/)
...
if (discordNativeModulesPath) {
    ...
    } catch (e) { console.warn("[QuasarCord] Erreur lors du scan des modules natifs:", e.message); }
}
try {
...
console.log("[QuasarCord] mainAppDirname:", global.mainAppDirname);
...
try {
    ...
    if (fs.existsSync(nativeModulesDir) && !buildInfo.localModulesRoot) {
        ...
        console.log("[QuasarCord] build_info.json patché → localModulesRoot:", nativeModulesDir);
    }
} catch (e) {
    console.warn("[QuasarCord] Impossible de patcher build_info.json:", e.message);
}
