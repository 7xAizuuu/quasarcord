 ...
 *
 * Usage:
 *   pnpm inject    → installe QuasarCord dans le Discord choisi
 *   pnpm uninject  → désinstalle QuasarCord du Discord choisi
...
// EquilotlCli est l'installeur graphique d'Equicord — on le réutilise pour QuasarCord
...
async function ensureBinary() {
    ...
    if (existsSync(outputFile)) {
        console.log("[QuasarCord] Installeur déjà présent, utilisation locale.");
        return outputFile;
    }

    console.log("[QuasarCord] Téléchargement de l'installeur (" + filename + ")...");

    const res = await fetch(BASE_URL + filename, {
        headers: {
            "User-Agent": "QuasarCord (https://github.com/7xAizuuu/quasarcord)"
        }
    });
    ...
    if (process.platform === "darwin") {
        console.log("[QuasarCord] Sauvegarde du zip...");
        const zip = new Uint8Array(await res.arrayBuffer());
        writeFileSync(downloadName, zip);

        console.log("[QuasarCord] Extraction du bundle...");
        execSync(`ditto -x -k '${downloadName}' '${FILE_DIR}'`);

        console.log("[QuasarCord] Suppression de la quarantaine macOS...");
        const logAndRun = cmd => {
    ...
    console.log("[QuasarCord] Installeur téléchargé avec succès !");
    return outputFile;
}
...
function checkBuild() {
    ...
    if (!existsSync(patcherPath)) {
        console.error("\x1b[31m[QuasarCord] dist/desktop/patcher.js introuvable !\x1b[0m");
...
function cleanOldQuasarCord() {
    console.log("[QuasarCord] Recherche et nettoyage automatique des anciennes installations...");
    ...
    for (const resourcesDir of candidates) {
        ...
        try {
            ...
            // 1. Supprimer le dossier app/ s'il a été créé par l'ancien QuasarCord
            if (existsSync(appDirPath)) {
                ...
                if (shouldDelete) {
                    console.log(`[QuasarCord] Suppression de l'ancien dossier app/ dans : ${resourcesDir}`);
            ...
            if (existsSync(backupPath)) {
                ...
                if (isAsarDir) {
                    console.log(`[QuasarCord] Suppression du dossier app.asar temporaire dans : ${resourcesDir}`);
                    rmSync(appAsarPath, { recursive: true, force: true });
                }

                if (!existsSync(appAsarPath) || isAsarDir) {
                    console.log(`[QuasarCord] Restauration _app.asar -> app.asar dans : ${resourcesDir}`);
                    ...
                    console.log(`[QuasarCord] Nettoyage du backup _app.asar obsolète dans : ${resourcesDir}`);
                    rmSync(backupPath, { force: true });
                    cleanedAny = true;
                }
            }
        } catch (e) {
            console.error(`[QuasarCord] Erreur lors du nettoyage de ${resourcesDir} :`, e.message);
        }
    }

    if (cleanedAny) {
        console.log("[QuasarCord] Nettoyage des anciennes installations terminé avec succès !");
    } else {
        console.log("[QuasarCord] Aucune ancienne installation à nettoyer.");
    }
}

// ── Main ─────────────────────────────────────────────────────────────────────
// On nettoie d'abord les anciennes traces de QuasarCord pour éviter tout conflit ou blocage
cleanOldQuasarCord();
...
if (isInstall) {
    console.log("[QuasarCord] Nettoyage des installations précédentes (Vencord/Equicord/QuasarCord)...");
    try {
        ...
        console.log("[QuasarCord] Nettoyage terminé.");
    } catch {
        console.log("[QuasarCord] Aucun mod précédent à nettoyer ou échec du nettoyage.");
    }
}

console.log("[QuasarCord] Lancement de l'injection...");
...
try {
    execFileSync(installerBin, args, {
        ...
        env: {
            ...
            EQUICORD_DIRECTORY: join(BASE_DIR, "dist", "desktop"),
            EQUICORD_DEV_INSTALL: "1",
            QUASARCORD_DIRECTORY: join(BASE_DIR, "dist", "desktop")
        }
    });
} catch {
    console.error("[QuasarCord] Erreur lors de l'injection.");
    process.exit(1);
}
