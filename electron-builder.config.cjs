...
// ─── Configuration de Build QuasarCord ─────────────────────────────────────────

function killQuasarCord() {
    const releaseDir = join(__dirname, "release", "quasarcord-dist");
...
function buildQuasarCordFromDiscord(discordApp) {
    ...
    console.log("[quasarcord] Préparation de _app.asar...");
...
app.setPath("userData", path.join(app.getPath("appData"), "QuasarCord"));
...
`);
    ...
    writeFileSync(join(outApp, "package.json"), JSON.stringify({ name: "quasarcord", main: "index.js", version: "1.18.0" }, null, 2));
...
`);
    ...
    const quasarcordPreload = join(__dirname, "quasarcord-preload.js");
    if (existsSync(quasarcordPreload)) {
        cpSync(quasarcordPreload, join(outDist, "preload.js"));
    }
    ...
    const iconSrc = join(__dirname, "quasarcord.ico");
    if (existsSync(iconSrc)) {
        ...
        try {
            ...
            if (existsSync(rcedit)) {
                execSync(`"${rcedit}" "${discordExe}" --set-icon "${iconSrc}" --set-version-string "ProductName" "QuasarCord" --set-version-string "FileDescription" "QuasarCord"`, { stdio: "ignore" });
            }
        } catch (e) { }
    }

    console.log(`[quasarcord] Build terminé -> ${outDir}`);
}
...
buildQuasarCordFromDiscord(discord);

module.exports = {
    appId: "com.quasarcord.app",
    productName: "QuasarCord",
    copyright: "Copyright 2026 QuasarCord",
